...
```{r}
murine_rl=lm(formula= Reestimulacion~Grupo,data=dataMurineNumeric)
summary(murine_rl)
```
...
par(mar = c(5, 5, 4, 14))
plot(dataMurineNumeric$Grupo,dataMurineNumeric$Reestimulacion, xlab="Grupo", ylab="Reestimulación")
abline(murine_rl)
...
```{r}
murine_rm<-lm(dataMurineNumeric$Reestimulacion~dataMurineNumeric$Grupo + dataMurineNumeric$Region+dataMurineNumeric$Antigeno,data=dataMurineNumeric[2:5])
summary(murine_rm)
```
...
```{r}
#definimos los vectores de las variables antigeno y raton y los guardamos en un dataframe
antigeno_murine<-c(dataMurineNumeric$Antigeno)
raton_murine<-c(dataMurineNumeric$Raton)
...
```{r}
grupos<-c(dataMurineNumeric$Grupo)
reestimulacion<-c(dataMurineNumeric$Reestimulacion)
df_murine_anova<-data.frame(grupos,reestimulacion) #definimos el conjunto de datos
head(df_murine_anova,4) #mostramos la información del data frame

```

```{r}
table(df_murine_anova$grupo) #número de observaciones por cada grupo
```

```{r}

##Media de reestimuación por grupos de tratamiento
aggregate(reestimulacion~grupos,data=df_murine_anova,FUN=mean)
##Desviación típica de reestimulación por grupos de tratamiento
aggregate(reestimulacion~grupos,data=df_murine_anova,FUN=sd)

library(ggplot2)

plotMurine <- ggplot(dataMurineNumeric, aes(x=Grupo, y=Reestimulacion)) + 
  geom_boxplot()+stat_summary(fun = median)
plotMurine + facet_wrap( ~ Grupo, scales="free")
...
par(mfrow=c(3,3))
qqnorm(df_murine_anova[df_murine_anova$grupos==1,"reestimulacion"],main=1)
qqline(df_murine_anova[df_murine_anova$grupos==1,"reestimulacion"])
qqnorm(df_murine_anova[df_murine_anova$grupos==2,"reestimulacion"],main=2)
qqline(df_murine_anova[df_murine_anova$grupos==2,"reestimulacion"])
qqnorm(df_murine_anova[df_murine_anova$grupos==3,"reestimulacion"],main=3)
qqline(df_murine_anova[df_murine_anova$grupos==3,"reestimulacion"])
qqnorm(df_murine_anova[df_murine_anova$grupos==4,"reestimulacion"],main=4)
qqline(df_murine_anova[df_murine_anova$grupos==4,"reestimulacion"])
qqnorm(df_murine_anova[df_murine_anova$grupos==5,"reestimulacion"],main=5)
qqline(df_murine_anova[df_murine_anova$grupos==5,"reestimulacion"])
qqnorm(df_murine_anova[df_murine_anova$grupos==6,"reestimulacion"],main=6)
qqline(df_murine_anova[df_murine_anova$grupos==6,"reestimulacion"])
...
require(nortest)
## Utilizamos dicho paquetae sobre el dataframe anterior
by(data=df_murine_anova,INDICES =
df_murine_anova$grupos,FUN=function(x){lillie.test(x$reestimulacion)})
```

```{r}
##Test Fligner-Killen diagnosis homocedasticidad
fligner.test(df_murine_anova$reestimulacion~df_murine_anova$grupos,df_murine_anova)
```

```{r}
##Estudio anova
anova_murine<-aov(df_murine_anova$reestimulacion~df_murine_anova$grupos,data=df_murine_anova)
...
```{r}
pairwise.t.test(x=df_murine_anova$reestimulacion,g=df_murine_anova$grupos,p.adjust.method ="holm",pool.sd=TRUE,paired=FALSE,alternative="two.sided")
```
...
## Apartado 8
```{r echo=FALSE}
cat("1 - La mayor correlacion de reestimulacion entre antigenos se encuentran en la misma region, estos son del mismo tipo y responden a los mismos factores.")
round(cor(dataMurineCorrelacion),2)

cat("2 - No todos los antigenos tienen más respuesta en los grupo con tumor, se debe a que algunos de los antigenos solo tienen reestimulacion con el tratamiento A (1,2,3,8) y otros con el tratamiento B (4,5,6,7,9")
probData
