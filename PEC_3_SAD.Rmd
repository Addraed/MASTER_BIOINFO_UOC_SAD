...
dataMurine$Antigeno<- factor(dataMurine$Antigeno,levels = c("1", "2","3"))
...
dataMurine$Region<-as.numeric(dataMurine$Region)
dataMurine$Antigeno<-as.numeric(dataMurine$Antigeno)
summary(dataMurine) 
```
...
```{r}
murine_rl=lm(formula= Reestimulacion~Grupo,data=dataMurine)
summary(murine_rl)
```
...
par(mar = c(5, 5, 4, 14))
plot(dataMurine$Grupo,dataMurine$Reestimulacion, xlab="Grupo", ylab="Reestimulación")
abline(murine_rl)
...
```{r}
murine_rm<-lm(dataMurine$Reestimulacion~dataMurine$Grupo + dataMurine$Region+dataMurine$Antigeno,data=dataMurine[2:5])
summary(murine_rm)
```
...
murine_rm<-lm(formula = dataMurine$Reestimulacion~dataMurine$Region + dataMurine$Antigeno, data = dataMurine[2:5])
summary(murine_rm)
```

```{r}
#definimos los vectores de las variables lage y lbase y los guardamos en un dataframe
antigeno_murine<-c(dataMurine$Antigeno)
raton_murine<-c(+dataMurine$Raton)
...
```{r}
grupos<-c(dataMurine$Grupo)
reestimulacion<-c(dataMurine$Reestimulacion)
...
par(mar = c(5, 5, 4, 14))
boxplot(dataMurine$Reestimulacion~dataMurine$Grupo, data=dataMurine, id.method="y")
legend(x = "topright",inset = c(-0.8,0),legend=c("1= Tratamiento A con tumor","2= Tratamiento A sin tumor","3= Tratamiento B con tumor","4= Tratamiento B sin tumor","5= Animal control con tumor","6= Animal control sin tumor"), xpd = TRUE)
